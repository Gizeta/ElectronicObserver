	...
	public class FleetData : APIWrapper, IIdentifiable {
		...
		public override void LoadFromResponse( string apiname, dynamic data ) {
			...
			switch ( apiname ) {
				...
				case "api_port/port":
					...
					if ( IsInSortie ) {
						Utility.Logger.Add( 2, string.Format( Properties.Resources.FleetData_ReturnInfo, FleetID, Name ) );
					}
					IsInSortie = false;
		...
		public static FleetStates UpdateFleetState( FleetData fleet, ImageLabel label, ToolTip tooltip, FleetStates prevstate, ref DateTime timer ) {
			...
			if ( fleet == null || fleet.Members.Count( id => id != -1 ) == 0 ) {
				label.Text = Properties.Resources.FleetData_FleetNoShip;
			...
			{	//入渠中
				...
				if ( ntime > 0 ) {	//入渠中
					...
					label.Text = Properties.Resources.FleetData_Repairing + DateTimeHelper.ToTimeRemainString( timer );
					label.ImageIndex = (int)ResourceManager.IconContent.FleetDocking;

					tooltip.SetToolTip( label, Properties.Resources.FleetData_RepairCompleteTime + timer );
			...
			if ( fleet.IsInSortie ) {
				...
				if ( fleet.MembersInstance.Count( s =>
					...
					label.Text = Properties.Resources.FleetData_LowHPInSortie;
					...
					label.Text = Properties.Resources.FleetData_InSortie;
			...
			if ( fleet.ExpeditionState != 0 ) {
				...
				label.Text = Properties.Resources.FleetData_Expediting + DateTimeHelper.ToTimeRemainString( timer );
				label.ImageIndex = (int)ResourceManager.IconContent.FleetExpedition;

				tooltip.SetToolTip( label, string.Format( Properties.Resources.FleetData_ExpeditionCompleteTime, KCDatabase.Instance.Mission[fleet.ExpeditionDestination].ID, KCDatabase.Instance.Mission[fleet.ExpeditionDestination].Name, timer ) );
			...
			if ( fleet.MembersInstance.Count( s =>
				...
				label.Text = Properties.Resources.FleetData_LowHPWarning;
			...
			//泊地修理中
			{
				if ( fleet.IsAnchorageRepairing ) {
					...
					label.Text = Properties.Resources.FleetData_AnchorageRepairing + DateTimeHelper.ToTimeElapsedString( KCDatabase.Instance.Fleet.AnchorageRepairingTimer );
					label.ImageIndex = (int)ResourceManager.IconContent.FleetAnchorageRepairing;

					tooltip.SetToolTip( label, string.Format( Properties.Resources.FleetData_AnchorageRepairTime, KCDatabase.Instance.Fleet.AnchorageRepairingTimer ) );
			...
			//未補給
			{
				...
				if ( fuel > 0 || ammo > 0 || bauxite > 0 ) {
					...
					label.Text = Properties.Resources.FleetData_NotSupplied;
					label.ImageIndex = (int)ResourceManager.IconContent.FleetNotReplenished;

					tooltip.SetToolTip( label, string.Format( Properties.Resources.FleetData_SupplyInfo, fuel, ammo, bauxite, aircraft ) );
			...
			//疲労
			{
				...
				if ( cond < Configuration.Config.Control.ConditionBorder && fleet.ConditionTime != null ) {
					...
					label.Text = Properties.Resources.FleetData_Tired + DateTimeHelper.ToTimeRemainString( timer );
					...
					tooltip.SetToolTip( label, string.Format( Properties.Resources.FleetData_TiredRestTime, timer ) );
					...
					label.Text = Properties.Resources.FleetData_Sparkle;
					label.ImageIndex = (int)ResourceManager.IconContent.ConditionSparkle;
					tooltip.SetToolTip( label, string.Format( Properties.Resources.FleetData_SparkleInfo, cond, Math.Ceiling( ( cond - 49 ) / 3.0 ) ) );
			...
			//出撃可能！
			{
				label.Text = Properties.Resources.FleetData_Ready;
