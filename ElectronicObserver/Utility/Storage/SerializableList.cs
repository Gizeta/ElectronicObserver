	...
	public class SerializableList<T> where T : IConvertible {
		...
		public static List<T> StringToList( string serial, bool suppressError = false ) {
			...
			try {
				...
				if ( !suppressError )
					Utility.ErrorReporter.SendErrorReport( ex, Properties.Resources.SerializableList_StringToListError );
			}
		...
		public static string ListToString( List<T> value, bool suppressError = false ) {
			...
			try {
				...
				if ( !suppressError )
					Utility.ErrorReporter.SendErrorReport( ex, Properties.Resources.SerializableList_ListToStringError );
			}
