	...
	public partial class FormCompass : DockContent {
		...
		private class TableEnemyMemberControl {
			...
			private string GetShipString( int shipID, int[] slot, int level, int hp, int firepower, int torpedo, int aa, int armor, int asw, int evasion, int los, int luck ) {
				...
				return string.Format(
							Properties.Resources.FormCompass_ShipInfo,
			...
			private string GetEquipmentString( int shipID, int[] slot ) {
				...
				sb.AppendFormat( Properties.Resources.FormCompass_AttackInfo,
...

				{
					...
					if ( aacutin != 0 ) {
						sb.AppendFormat( Properties.Resources.FormCompass_AAInfo, Constants.GetAACutinKind( aacutin ) );
					}
				}
		...
		private void Updated( string apiname, dynamic data ) {
			...
			if ( apiname == "api_port/port" ) {
				...
				TextMapArea.Text = Properties.Resources.Constant_Practice;
				...
				TextMapArea.Text = string.Format( Properties.Resources.FormCompass_MapAreaInfo, compass.MapAreaID, compass.MapInfoID );
				
				TextDestination.Text = string.Format( Properties.Resources.FormCompass_NextPointInfo, compass.Destination, ( compass.IsEndPoint ? Properties.Resources.FormCompass_EndPoint : "" ) );
				if ( compass.LaunchedRecon != 0 ) {
					...
					ToolTipInfo.SetToolTip( TextDestination, Properties.Resources.FormCompass_Seaplane );
				} else {
...

				{
					...
					switch ( compass.EventID ) {
						...
						case 0:		//初期位置
						case 1:		//不明
							TextEventDetail.Text = Properties.Resources.FormCompass_UnknownEventID;
							break;
						...
						case 8:		//船団護衛成功
							{
								...
								if ( compass.GetItemID == 4 ) {		//"※"　大方資源専用ID
									...
									if ( item != null )
										...
										materialname = Properties.Resources.FormCompass_UnknownItem;
								}
						...
						default:
							eventkind += Properties.Resources.Constant_Unknown;
							TextEventDetail.Text = "";
							break;
		...
		private void UpdateEnemyFleet( int fleetID ) {
			...
			TextEventDetail.Text = string.Format( Properties.Resources.FormCompass_EnemyIDInfo, fleetID );
			...
			if ( !efleet.Record.ContainsKey( fleetID ) ) {
				...
				//unknown
				TextEnemyFleetName.Text = Properties.Resources.FormCompass_UnknownEnemy;
