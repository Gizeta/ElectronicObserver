...
namespace ElectronicObserver.Window {
	...
	public partial class FormInformation : DockContent {
		...
		private string GetPracticeEnemyInfo( dynamic data ) {
			...
			sb.AppendLine( Properties.Resources.FormInformation_Practice_Title );
			sb.AppendLine( Properties.Resources.FormInformation_Practice_EnemyFleetName + data.api_deckname );

			{
				...
				sb.AppendLine( Properties.Resources.FormInformation_Practice_GainExp + (int)expbase );
				sb.AppendLine( Properties.Resources.FormInformation_Practice_GainSExp + (int)( expbase * 1.2 ) );
		...
		private string GetAlbumInfo( dynamic data ) {
			...
			if ( data.api_list != null ) {
				...
				if ( data.api_list[0].api_yomi() ) {
					//艦娘図鑑
					sb.AppendLine( Properties.Resources.FormInformation_Album_MidHPNotCollected );
					...
					sb.AppendLine( Properties.Resources.FormInformation_Album_ShipNotCollected );
					...
					sb.AppendLine( Properties.Resources.FormInformation_Album_EquipmentNotCollected );
					for ( int i = 0; i < bound; i++ ) {
						if ( !flags[i] ) {
		...
		private string GetCreateItemInfo( dynamic data ) {
			...
			if ( (int)data.api_create_flag == 0 ) {
				...
				StringBuilder sb = new StringBuilder();
				sb.AppendLine( Properties.Resources.FormInformation_CreateItem_Failed );
		...
		private string GetMapGauge( dynamic data ) {
			...
			StringBuilder sb = new StringBuilder();
			sb.AppendLine( Properties.Resources.FormInformation_Map_BossHP );
			...
			foreach ( dynamic elem in data ) {
				...
				if ( map != null ) {
					if ( map.RequiredDefeatedCount != -1 && elem.api_defeat_count() ) {
						...
						sb.AppendFormat( Properties.Resources.FormInformation_Map_BossDefeatInfo, map.MapAreaID, map.MapInfoID, (int)elem.api_defeat_count, map.RequiredDefeatedCount );
		...
		private string GetExpeditionResult( dynamic data ) {
			...
			sb.AppendLine( Properties.Resources.FormInformation_Expedition_Title );
			sb.AppendLine( data.api_quest_name );
			sb.AppendFormat( Properties.Resources.FormInformation_Expedition_ResultInfo, Constants.GetExpeditionResult( (int)data.api_clear_result ) );
			sb.AppendFormat( Properties.Resources.FormInformation_Expedition_AdmiralExpInfo, (int)data.api_get_exp );
			sb.AppendFormat( Properties.Resources.FormInformation_Expedition_ShipExpInfo, ( (int[])data.api_get_ship_exp ).Min() );
