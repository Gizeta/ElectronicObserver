	...
	public sealed class APIObserver {
		...
		public int Start( int portID, Control UIControl ) {
			...
			*/
			...
			Utility.Logger.Add( 2, string.Format( Properties.Resources.APIObserver_Start, Fiddler.FiddlerApplication.oProxy.ListenPort ) );


			//checkme: 一応警告をつけてみる
			if ( portID != Fiddler.FiddlerApplication.oProxy.ListenPort ) {
				Utility.Logger.Add( 3, Properties.Resources.APIObserver_MismatchedPort );
			}
		...
		public void Stop() {
			...
			Utility.Logger.Add( 2, Properties.Resources.APIObserver_Stop );
		}
			...
			//保存
			{
				...
				if ( c.SaveReceivedData ) {
					...
					try {
						...
						if ( c.SaveResponse && oSession.fullUrl.Contains( "/kcsapi/" ) ) {
							...
							Task.Run( (Action)( () => {
								try {
									...
									Utility.Logger.Add( 1, string.Format( Properties.Resources.APIObserver_Saved, tpath.Remove( 0, saveDataPath.Length + 1 ) ) );

								} catch ( IOException ex ) {	//ファイルがロックされている; 頻繁に出るのでエラーレポートを残さない

									Utility.Logger.Add( 3, Properties.Resources.APIObserver_SaveError + ex.Message );
								}
							} ) );
						...
						Utility.ErrorReporter.SendErrorReport( ex, Properties.Resources.APIObserver_SaveError );
					}
		...
		public void LoadRequest( string path, string data ) {
			...
			try {
				...
				Utility.Logger.Add( 1, Properties.Resources.APIObserver_RequestReceive + shortpath );
				...
				ErrorReporter.SendErrorReport( ex, Properties.Resources.APIObserver_RequestError, shortpath, data );
		...
		public void LoadResponse( string path, string data ) {
			...
			try {
				...
				Utility.Logger.Add( 1, Properties.Resources.APIObserver_ResponseReceive + shortpath );
				...
				if ( (int)json.api_result != 1 ) {
					...
					var ex = new ArgumentException( Properties.Resources.APIObserver_ReceiveErrorData );
					Utility.ErrorReporter.SendErrorReport( ex, Properties.Resources.APIObserver_ReceiveErrorData );
					throw ex;
				}
				...
				ErrorReporter.SendErrorReport( ex, Properties.Resources.APIObserver_ResponseError, shortpath, data );
		...
		private void SaveRequest( string url, string body ) {
			...
			try {
				...
				Utility.ErrorReporter.SendErrorReport( ex, Properties.Resources.APIObserver_RequestSaveError );
		...
		private void SaveResponse( string url, string body ) {
			...
			try {
				...
				Utility.ErrorReporter.SendErrorReport( ex, Properties.Resources.APIObserver_ResponseSaveError );
